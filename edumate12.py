            ...
            if tnc.get(): # checking for acceptance of agreement 
                if re.match("^.+@(\[?)[a-zA-Z0-9-.]+.([a-zA-Z]{2,3}|[0-9]{1,3})(]?)$", email.get()): # validating the email 
                    if password.get() == repassword.get():
                        ...
                        select_query =  "SELECT email FROM student_details where email = '" + email.get()+"' ;" # queries for retrieving values 
                        ...
                        if student_info:
                             Label(screen1, text="Email Exist", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570) 
                             return
                        else:
...
def welcome_page(student_info): 
    ...
    Button(screen2, text='Enter your grades', width=20, font=("Open Sans", 13, 'bold'), bg='red', fg='white', command=student_new_record).place(x=270, y=250) 
    Button(screen2, text='Check your result', width=20, font=("Open Sans", 13, 'bold'), bg='red', fg='white',command=student_record).place(x=270, y=350) 
